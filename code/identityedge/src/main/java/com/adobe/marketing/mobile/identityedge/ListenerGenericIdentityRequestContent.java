...
class ListenerGenericIdentityRequestContent extends ExtensionListener {
     ...
     */
    ListenerGenericIdentityRequestContent(final ExtensionApi extensionApi, final String type, final String source) {
    ...
    public void hear(final Event event) {
        if (event == null || event.getEventData() == null) {
            MobileCore.log(LoggingMode.DEBUG, IdentityEdgeConstants.LOG_TAG, "Event or Event data is null. Ignoring the event listened by ListenerGenericIdentityRequestContent");
            return;
        }
        ...
        if (parentExtension == null) {
            MobileCore.log(LoggingMode.DEBUG, IdentityEdgeConstants.LOG_TAG,
                    "The parent extension, associated with the ListenerGenericIdentityRequestContent is null, ignoring the configuration response content event.");
            return;
        }

        parentExtension.handleGenericIdentityRequest(event);
    }
